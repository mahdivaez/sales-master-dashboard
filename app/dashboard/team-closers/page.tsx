...
export default function TeamClosersPage() {
  ...
  const fetchTeamMembers = async () => {
    try {
      const response = await fetch('/api/team?role=closer');
  ...
  // Filter and sort by closer amount
  const filteredMembers = members
    .filter(m => m.name.toLowerCase().includes(searchTerm.toLowerCase()))
    .sort((a, b) => b.stats.closerAmount - a.stats.closerAmount);

  const totalClosers = members.length;
  const totalCloserRevenue = members.reduce((sum, m) => sum + m.stats.closerAmount, 0);
  const totalCloserCount = members.reduce((sum, m) => sum + m.stats.closerCount, 0);
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="p-3 bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl shadow-lg shadow-amber-500/25 border-2 border-amber-400/30">
                <Crown className="w-7 h-7 text-white" />
              </div>
              <div>
                <h1 className="text-4xl font-black text-gray-900 tracking-tight">Team Closers</h1>
                <p className="text-sm font-semibold text-amber-600 uppercase tracking-wider">Performance Rankings</p>
              </div>
            </div>
            <p className="text-gray-500 font-medium ml-14">Track closer performance and revenue</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-2xl p-6 border-2 border-gray-200 shadow-lg">
            ...
            <p className="text-gray-500 font-medium">Total Closers</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 border-2 border-gray-200 shadow-lg">
            ...
            <p className="text-3xl font-black text-gray-900">${totalCloserRevenue.toLocaleString()}</p>
          ...
          <div className="bg-white rounded-2xl p-6 border-2 border-gray-200 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-blue-500 text-sm font-bold flex items-center gap-1">
                <BarChart3 className="w-4 h-4" /> Deals
              </span>
            </div>
            <p className="text-3xl font-black text-gray-900">{totalCloserCount}</p>
            <p className="text-gray-500 font-medium">Total Deals Closed</p>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-12 pr-6 py-3 bg-white border-2 border-gray-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-200 outline-none transition-all font-medium shadow-md"
            />
          </div>
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-50/50">
                  ...
                  <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Deals Closed</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center justify-center">
                        <div className="w-10 h-10 border-4 border-gray-200 border-t-amber-500 rounded-full animate-spin mb-4" />
                        <p className="text-gray-500 font-medium">Loading closers...</p>
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <p className="text-gray-500 font-medium">No closers found</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={member.id} className="hover:bg-amber-50/30 transition-colors">
                      ...
                      <td className="px-6 py-5 text-center">
                        <Link
                          ...
                          className="inline-flex items-center gap-1 px-3 py-1.5 bg-amber-100 text-amber-700 rounded-lg font-bold text-sm hover:bg-amber-200 transition-colors"
                        >
                          View Details
